...
const indenting = function () {
	...
	if (log_indent_level > 0) {
		for (let x = 1, end = log_indent_level, asc = 1 <= end; asc ? x <= end : x >= end; asc ? x++ : x--) {
			str += " | ";
		}
	}
	return str;
...
// Helper functions
const log_entry = function (str, level) {
	if (level == null) {
		level = levels.normal;
	}
	return log(str, level, "normal");
};
const log_info = function (str, level) {
	if (level == null) {
		level = levels.info;
	}
	return log(str, level, "info");
};
const log_event = function (str, level) {
	if (level == null) {
		level = levels.event;
	}
	return log(str, level, "event");
};
const log_game = function (str, level) {
	if (level == null) {
		level = levels.game;
	}
	return log(str, level, "game");
};
const log_warning = function (str, level) {
	if (level == null) {
		level = levels.warning;
	}
	return log(str, level, "warning");
};
const log_error = function (str, level) {
	if (level == null) {
		level = levels.normal;
	}
	return log(str, level, "error");
};
const log_fatal = function (str, level) {
	if (level == null) {
		level = levels.normal;
	}
	return log(str, level, "fatal");
};
const log_debug = function (str, level) {
	if (level == null) {
		level = levels.debug;
	}
	return log(str, level, "debug");
};
const log_verbose = function (str, level) {
	if (level == null) {
		level = levels.verbose;
	}
	return log(str, level, "verbose");
};
const log_important = function (str, level) {
	if (level == null) {
		level = levels.normal;
	}
	return log(str, level, "important");
};
const log_update = function (str, level) {
	if (level == null) {
		level = levels.normal;
	}
	return log(str, level, "update");
};
const log_end = function (str, level) {
	if (level == null) {
		level = levels.normal;
	}
	return log(str, level, "end");
};
var log_raw = function (str, level) {
	if (level == null) {
		level = levels.normal;
	}
	return log(str, level, "raw");
};
var log_meta = function (str, level) {
	if (level == null) {
		level = levels.verbose;
	}
	return log(str, level, "meta");
};
...
log_indent.entry = function (str, level) {
	log_indent();
	log_entry(str, level);
	return log_outdent();
};
log_indent.info = function (str, level) {
	log_indent();
	log_info(str, level);
	return log_outdent();
};
log_indent.event = function (str, level) {
	log_indent();
	log_event(str, level);
	return log_outdent();
};
log_indent.game = function (str, level) {
	log_indent();
	log_game(str, level);
	return log_outdent();
};
log_indent.warning = function (str, level) {
	log_indent();
	log_warning(str, level);
	return log_outdent();
};
log_indent.error = function (str, level) {
	log_indent();
	log_error(str, level);
	return log_outdent();
};
log_indent.fatal = function (str, level) {
	log_indent();
	log_fatal(str, level);
	return log_outdent();
};
log_indent.debug = function (str, level) {
	log_indent();
	log_debug(str, level);
	return log_outdent();
};
log_indent.verbose = function (str, level) {
	log_indent();
	log_verbose(str, level);
	return log_outdent();
};
log_indent.important = function (str, level) {
	log_indent();
	log_important(str, level);
	return log_outdent();
};
log_indent.update = function (str, level) {
	log_indent();
	log_update(str, level);
	return log_outdent();
};
log_indent.end = function (str, level) {
	log_indent();
	log_end(str, level);
	return log_outdent();
};
log_indent.raw = function (str, level) {
	log_indent();
	log_raw(str, level);
	return log_outdent();
};
log_indent.meta = function (str, level) {
	log_indent();
	log_meta(str, level);
	return log_outdent();
};
