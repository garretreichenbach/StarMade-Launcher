...
app.run(function($q, $rootScope, $state, $timeout, accessToken, api, refreshToken, updater) {
  ...
  $rootScope.log.indent();
  let _flags   = [];
  for (var arg of Array.from(Object.keys(argv).slice(1))) { _flags.push(`--${arg}`); }
  _flags   = _flags.join(" ");
  ...
  $rootScope.log.indent();
  if ($rootScope.debugging) { $rootScope.log.entry("Debugging: enabled" + ($rootScope.verbose ? " (verbose)" : "")); }
  ...
  $rootScope.alreadyExecuted = function(id, cooldown) {
    ...
    if (!$rootScope.alreadyExecuted.ids) { $rootScope.alreadyExecuted.ids = {}; }
    ...
    // Already executed?
    if (expiry <= 0) { return true; }           // <1: only execute once
    if (expiry >= Date.now()) { return true; }  // still within cooldown
      ...
      .then(function(versions) {
        if (versions[0].version !== pkg.version) {
          ...
          if (process.platform === 'darwin') {
            ...
            if (process.platform === 'win32') { launcherExec += '.exe'; }
          }
    ...
    if (args.playerName != null) {
      ...
      return api.getCurrentUser()
        .then(function(response) {
          ...
          if (data.user.steam_link != null) { scope.steamAccountLinked = true; }
  ...
  if (!argv.nogui) {
    if (!$rootScope.noUpdate) { launcherAutoupdate(); }
