...
let javaJreDirectory = util.getJreDirectory(javaVersion);
...
app.controller('LaunchCtrl', function($scope, $rootScope, $timeout, accessToken) {
  ...
  const loadMemorySettings = function() {
    ...
    if ((_max > defaults[os.arch()].ceiling) && _do_logging) { $rootScope.log.indent.info("Max memory capped to physical ram"); }
    ...
    $scope.memory.validate = function() {
      ...
      if (!$scope.memory.validate.initial()) { return false; }
      if (!$scope.memory.validate.earlyGen()) { return false; }
      if (!$scope.memory.validate.max()) { return false; }
      if     ($scope.memory.earlyGen >= $scope.memory.initial) { return false; }
      if     ($scope.memory.max      <  ($scope.memory.initial + $scope.memory.earlyGen)) { return false; }
      if     ($scope.memory.max      >  $scope.memory.ceiling) { return false; }
      return true;
    };

    $scope.memory.validate.max = function() {
      // catch `undefined` from invalid values
      if (($scope.memory.max == null)) { return false; }
      if (!typeof $scope.memory.max === "number") { return false; }

      _max = $scope.memory.max;
      if     (_max >  $scope.memory.ceiling) { return false; }
      if     (_max < ($scope.memory.initial + $scope.memory.earlyGen)) { return false; }
      return true;
    };

    $scope.memory.validate.initial = function() {
      if (($scope.memory.initial == null)) { return false; }    // catch `undefined` from invalid values
      return true;
    };

    $scope.memory.validate.earlyGen = function() {
      if (($scope.memory.earlyGen == null)) { return false; }   // catch `undefined` from invalid values
      return true;
    };


    $scope.memory.validate.max.class      = function() {
      if (!$scope.memory.validate.max()) { return "invalid"; }
      if     ($scope.memory.max < 2048) { return "critical"; }
      if     ($scope.memory.max < 4096) { return "warning"; }
    };
    $scope.memory.validate.initial.class  = function() {
      if (!$scope.memory.validate.initial()) { return "invalid"; }
      if     ($scope.memory.initial <= $scope.memory.earlyGen) { return "invalid"; }
      if     ($scope.memory.initial <  256) { return "warning"; }
    };
    return $scope.memory.validate.earlyGen.class = function() {
      if (!$scope.memory.validate.earlyGen()) { return "invalid"; }
    };
  };
  ...
  $scope.resetJavaArgs = function() {
    let args = [];
    args.push('-Xincgc');
    if (os.arch() === "x64") { args.push('-server'); }
    args = args.join(" ");
    // Don't bother if they've already been reset
    if ($scope.launcherOptions.javaArgs === args) { return; }
  ...
  $scope.setJavaArgs = function() {
    if ($scope.launcherOptions.javaArgs === localStorage.getItem("javaArgs")) { return; }
  ...
  $scope.showEnvJavaOptionsWarning = function() {
    ...
    if ($rootScope.alreadyExecuted('log _JAVA_OPTIONS Dialog')) { return; }
  ...
  $scope.$watch('memory.earlyGen', function(newVal) {
    if ((document.getElementById("maxMemorySlider") == null)) { return; }  // Ensure markup has loaded
    if (typeof $scope.memory === "undefined") { return; }
  ...
  var update_slider_class = function() {
    ...
    if  (pow2) { document.getElementById("maxMemorySlider").classList.add("power-of-2"); }
    if (!pow2) { return document.getElementById("maxMemorySlider").classList.remove("power-of-2"); }
  };
  ...
  var nearestPow2 = function(val, memoize) {
    ...
    if (memoize && (typeof pow2_lower_bound === "number")  &&  (typeof pow2_upper_bound === "number")) {  // Skip entire block if bounds are undefined/incorrect
      // no change?
      if (val === pow2_current_power) { return pow2_current_power; }
    ...
    if ((val & (val-1)) === 0) { return val; }  // This will be nonzero (and therefore fail) if there are multiple bits set.
  ...
  var updateMemorySlider = function(earlyGen, initial) {
    ...
    if (typeof earlyGen === "undefined") { return; }
    if (typeof initial  === "undefined") { return; }

    if (!$rootScope.alreadyExecuted("Log - updateMemorySlider", 1000)) { _do_logging = true; }
  ...
  $scope.verifyJavaPath = () => {
    ...
    if (!$rootScope.alreadyExecuted('log verifyJavaPath', 1000)) { _do_logging = true; }
  ...
  return $scope.launch = dedicatedServer => {
    ...
    if (dedicatedServer == null) { dedicatedServer = false; }
    ...
    let stdio  = 'inherit';
    if ($rootScope.captureGame && !detach) { stdio  = 'pipe'; }
    ...
    const args = [];
    // JVM args
    if ($rootScope.verbose) { args.push('-verbose:jni'); }
    ...
    // Custom args
    for (var arg of Array.from($scope.launcherOptions.javaArgs.split(" "))) { args.push(arg); }
    // Jar args
    args.push('-jar');
    args.push(starmadeJar);
    if (!dedicatedServer) { args.push('-force'); }
   // args.push('-server')                         if dedicatedServer
    if (dedicatedServer) { args.push('-gui'); }
    args.push(`-port:${$scope.serverPort}`);
    if (accessToken.get() != null) { args.push(`-auth ${accessToken.get()}`); }


    // Debug output
    $rootScope.log.debug("Command:");
    const command = javaExec + " " + args.join(" ");
    for (var cmd_slice of Array.from(command.match(/.{1,128}/g))) { $rootScope.log.indent.debug(cmd_slice); }
    ...
    for (var envvar of Array.from(Object.keys(process.env))) { $rootScope.log.indent.verbose(`  ${envvar} = ${process.env[envvar]}`); }
    $rootScope.log.outdent();
    ...
    if ($rootScope.captureGame && !detach) {
      ...
      child.stdout.on('data', function(data) {
        let str = "";
        for (var char of Array.from(data)) { str += String.fromCharCode(char); }
