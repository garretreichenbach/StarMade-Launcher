...
app.controller('UpdateCtrl', function ($filter, $rootScope, $scope, $q, $timeout, updater, updaterProgress) {
	...
	if ($scope.backupOptions.configs === null) {
		$scope.backupOptions.configs = true;
	}
	if ($scope.backupOptions.worlds === null) {
		$scope.backupOptions.worlds = true;
	}
	if ($scope.backupOptions.blueprints === null) {
		$scope.backupOptions.blueprints = true;
	}
	if ($scope.backupOptions.blueprints === null) {
		$scope.backupOptions.blueprints = true;
	}
	...
	$rootScope.$watch('launcherUpdating', function (updating) {
		if (!updating) {
			return branchChange($scope.branch);
		}
	});

	$scope.$watch('branch', function (newVal) {
		if ($rootScope.launcherUpdating) {
			return;
		}
	...
	const getInstalledVersion = function () {
		let _do_logging;
		if (!$rootScope.alreadyExecuted('log getInstalledVersion')) {
			_do_logging = true;
		}
	...
	var set_backup_compression = function (newVal) {
		if (localStorage.getItem('backupCompressionType') === newVal) {
			return;
		}
	...
	$scope.backup = function () {
		...
		if ($scope.backupOptions.compressionType === "zip") {
			backupPath += ".zip";
		}
		...
		const _error_handler = function (err) {
			...
			if ((typeof err !== 'string') && (Object.keys(err).length > 0)) {
				msgs = [];
				for (var key of Array.from(Object.keys(err))) {
					msgs.push(`${key}: ${err[key]}`);
				}
			}
			for (var msg of Array.from(msgs)) {
				$rootScope.log.indent.entry(msg);
		...
		if ($scope.backupOptions.configs) {
			...
			for (var config of Array.from(configs)) {
				...
				if (!fileExists(path.resolve(path.join($scope.installDir, config)))) {
					continue;
				}
				_found = true;
				archive.file(
		...
		if ($scope.backupOptions.worlds) {
			...
			if (fileExists(path.resolve(path.join($scope.installDir, "server-database")))) {
				...
				$scope.backupDialog.progress.worlds = "missing";
			}
		} else {
			$scope.backupDialog.progress.worlds = "skipped";
		}
		...
		if ($scope.backupOptions.blueprints) {
			...
			if (fileExists(path.resolve(path.join($scope.installDir, "blueprints")))) {
				...
				$scope.backupDialog.progress.blueprints = "missing";
			}
		} else {
			$scope.backupDialog.progress.blueprints = "skipped";
		}
