...
app.service('updater', function($q, $http, Checksum, Version, $rootScope, updaterProgress) {
  ...
  this.update = function(version, installDir, checkOnly, force) {
    ...
    if (updaterProgress.inProgress) { return; }

    $rootScope.log.event("Updating game...");

    if (checkOnly) { $rootScope.log.indent.info("(checkOnly)"); }
    if (force) { $rootScope.log.indent.info("(forced)"); }
      ...
      .then(function(checksums) {
        ...
        const download = _.after(checksums.length, function() {
          ...
          if (checkOnly) {
            ...
            return filesToDownload.forEach(checksum => q.push(checksum, function(err) {
              if (err) { return $rootScope.log.error(err); }
            }));
          }
        });
            // $rootScope.log.outdent()
        ...
        return checksums.forEach(checksum => checksum.checkLocal(installDir)
          .then(function(needsDownloading) {
            if (needsDownloading || force) { filesToDownload.push(checksum); }
      ...
      try {
        return request(`${LAUNCHER_BASE_URL}/${sourceFilePath}/app.asar`)
          .on('response', function(response) {
            ...
            let msg =  null;
            if (response.statusCode === 401) { msg = "Not Authorized"; }
            if (response.statusCode === 403) { msg = "Forbidden"; }
            if (response.statusCode === 404) { msg = "Not found"; }
            if (response.statusCode === 500) { msg = "Internal Server Error"; }
            if (response.statusCode === 502) { msg = "Bad Gateway"; }
            if (response.statusCode === 503) { msg = "Service Unavailable"; }
            if (response.statusCode === 504) { msg = "Gateway Timeout"; }
            if (msg === null) {
  ...
  this.getChecksums = pathName => $q((resolve, reject) => $http.get(`${BASE_URL}/${pathName}/checksums`)
    .then(function(response) {
      ...
      return resolve(checksums);}).catch(response => // TODO: Consolidate this to one argument to be consist with how
  ...
  this.getVersions = branch => $q((resolve, reject) => $http.get(BRANCH_INDEXES[branch])
    .then(function(response) { // success
      ...
      lines.forEach(function(line) {
        if (line === '') { return; }                  // Skip blank     entries
        if (line.substring(0,1) === '#') { return; }  // Skip commented entries
        ...
        if (!buildPath || !buildVersion || !buildBuild) { return; }  // missing segments
        if (buildPath.indexOf('#') >= 0) { return; }                   // two entries on a line

        return versions.push(new Version(buildPath, buildVersion, buildBuild));
      });

      return resolve(uniqVersions(versions));}).catch(function(response) { // error
